...
Guide for AI coding agents (Claude Code, Cursor, Codex, and similar) working in this repo.
...
- `CONTRIBUTING.md` — entry point for org-wide contribution standards; an index that badge-links into the `docs/` pages below, plus a pointer to [Where documentation lives](docs/where-docs-live.md). Notes most rules are enforced via GitHub Rulesets + CI.
...
- Per-repo agent guides across the org are named `AGENTS.md` (not `CLAUDE.md`) — keep that name if you touch `documentation-information-architecture.md` / `where-docs-live.md` or any other doc that lists it.
